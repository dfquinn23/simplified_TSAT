...
""",
    ...
    def _prepare_report_context(
        ...
        context_parts.append(f"AUDIT DATE: {datetime.now().strftime('%B %d, %Y')}\n")
            ...
            if research.get('success'):
                ...
                if updates:
                    ...
                    for update in updates[:5]:  # Top 5
                        context_parts.append(f"  - {update.get('feature_name', 'Unknown')}\n")
                        context_parts.append(f"    Category: {update.get('update_category', 'N/A')}\n")
                        context_parts.append(f"    Automation Potential: {update.get('automation_potential', 'Unknown')}\n")
                        if update.get('business_impact'):
                            context_parts.append(f"    Impact: {update['business_impact']}\n")
            else:
                context_parts.append(f"Research Status: {research.get('error', 'No updates found')}\n")
